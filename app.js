...
function renderMiniTaskCard(t) {
  ...
  card.querySelector(".more-btn").addEventListener("click", (e) => {
    e.stopPropagation();
    openCardMenu(e.currentTarget, [
      { label: t.completed ? "Mark active" : "Mark complete", onClick: () => toggleComplete(t.id) },
      { label: "Edit", onClick: () => (t.daily ? openDailyModal(t.id) : openMiniModal(t.id)) },
      { label: "Info", onClick: () => showInfoModal("task", t.id) },
      { label: "Delete", danger: true, onClick: () => confirmDelete("task", t.id, t.title) },
    ]);
  });
...
function saveTaskFromPage() {
  ...
  if (editingTaskId) {
    ...
    if (t) {
      t.title = title; t.tags = tags; t.body = body; t.special = special; t.deadline = deadline; t.daily = daily;
    ...
    state.tasks.push({
      id: uid(), title, tags, body, createdAt: new Date().toISOString(), special,
...
function closeMiniModal() { el("#miniBackdrop").classList.remove("open"); editingMiniId = null; }
...
function saveMiniFromModal() {
  ...
  if (editingMiniId) {
    ...
    state.tasks.push({
      id: uid(), title, tags: [], body: "", createdAt: new Date().toISOString(), special: false,
...
/* ================= Daily task (resets to active every midnight) ================= */
let editingDailyId = null;

function openDailyModal(id) {
  editingDailyId = id || null;
  const t = id ? state.tasks.find((x) => x.id === id) : null;
  el("#dailySheetTitle").textContent = t ? "Edit daily task" : "Daily task";
  el("#dailyInput").value = t ? t.title : "";
  el("#dailyBackdrop").classList.add("open");
  setTimeout(() => el("#dailyInput").focus(), 50);
}

function closeDailyModal() { el("#dailyBackdrop").classList.remove("open"); editingDailyId = null; }

function saveDailyFromModal() {
  const title = el("#dailyInput").value.trim();
  if (!title) { el("#dailyInput").focus(); return; }
  if (editingDailyId) {
    const t = state.tasks.find((x) => x.id === editingDailyId);
    if (t) {
      t.title = title;
      if (!t.editHistory) t.editHistory = [];
      t.editHistory.push(new Date().toISOString());
    ...
    state.tasks.push({
      id: uid(), title, tags: [], body: "", createdAt: new Date().toISOString(), special: false,
      deadline: null, reminderAt: null, notified: false, completed: false, completedAt: null,
      deletedAt: null, editHistory: [], mini: true, daily: true,
    });
  }
  closeDailyModal();
  renderAll();
...
// Any daily task completed on a previous day resets to active — checked on load and periodically.
function resetDailyTasks() {
  const todayKey = dayKey(new Date());
  let changed = false;
  state.tasks.forEach((t) => {
    if (t.daily && !t.deletedAt && t.completed && t.completedAt && dayKey(t.completedAt) !== todayKey) {
      t.completed = false;
      t.completedAt = null;
      changed = true;
    }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Mini task quick-add
  el("#addMiniBtn").addEventListener("click", () => openMiniModal(null));
  el("#descToolbar").addEventListener("click", () => openMiniModal(null));
  ...
  el("#addDailyBtn").addEventListener("click", () => openDailyModal(null));
  el("#dailyBackdrop").addEventListener("click", (e) => { if (e.target.id === "dailyBackdrop") closeDailyModal(); });
  el("#dailyCancelBtn").addEventListener("click", closeDailyModal);
  el("#dailySaveBtn").addEventListener("click", saveDailyFromModal);
