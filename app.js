...
function renderTaskList() {
  ...
  let groups;
  if (activeTab === "active") {
    groups = groupByDay(tasks.filter((t) => !t.completed && matchesSearch(taskSearchText(t))), "createdAt");
...
function renderMiniTaskCard(t) {
  ...
  card.querySelector(".more-btn").addEventListener("click", (e) => {
    ...
    openCardMenu(e.currentTarget, [
      ...
      { label: "Edit", onClick: () => openMiniModal(t.id) },
      { label: "Info", onClick: () => showInfoModal("task", t.id) },
      { label: "Delete", danger: true, onClick: () => confirmDelete("task", t.id, t.title) },
    ]);
  });
...
function saveTaskFromPage() {
  ...
  if (editingTaskId) {
    ...
    if (t) {
      t.title = title; t.tags = tags; t.body = body; t.special = special; t.deadline = deadline;
    ...
    state.tasks.push({
      ...
      deadline, reminderAt, notified: false, completed: false, completedAt: null, deletedAt: null, editHistory: [], mini: false,
    });
  }
...
function saveMiniFromModal() {
  ...
  if (editingMiniId) {
    ...
    state.tasks.push({
      ...
      deletedAt: null, editHistory: [], mini: true,
    });
  }
  closeMiniModal();
  renderAll();
