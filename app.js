...
function renderTaskCard(t) {
  ...
  card.innerHTML = `
    ...
    <div class="more-wrap">
      ...
      <div class="dropdown">
        ...
        <button class="info-opt">Info</button>
  ...
  wireMoreMenu(card, () => openTaskSheet(t.id), () => confirmDelete("task", t.id, t.title), () => showInfoModal("task", t.id));

  return card;
}

function wireMoreMenu(card, onEdit, onDelete, onInfo) {
  ...
  const infoBtn = dropdown.querySelector(".info-opt");
  if (infoBtn && onInfo) infoBtn.addEventListener("click", (e) => { e.stopPropagation(); closeDropdowns(); onInfo(); });
}
...
function saveTaskFromSheet() {
  ...
  if (editingTaskId) {
    ...
    state.tasks.push({
      ...
      deadline, reminderAt, notified: false, completed: false, completedAt: null, deletedAt: null, editHistory: [],
    });
  }
